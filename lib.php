...
function local_boost_navdrawerfumbling_extend_navigation(global_navigation $navigation) {
    ...
    if (isset($config->removemycoursesnode) && $config->removemycoursesnode == true) {
        ...
        if ($mycoursesnode = $navigation->find('mycourses', global_navigation::TYPE_ROOTNODE)) {
            ...
            // Hide all courses below the mycourses nodeName
            $mycourseschildrennodes_keys = $mycoursesnode->get_children_key_list();
    ...
    if (isset($config->currentcoursefullname) && $config->currentcoursefullname == true) {
        ...
        if (!empty($PAGE->course->shortname) && !empty($PAGE->course->fullname)) {
            // Unfortunately, the navigation node with the course's shortname is contained in the flat_navigation object and not in the global_navigation object
            // We can't access flat_navigation here, so change the strings with a sledgehammer method
...
/**
 * Moodle core does not add a key to the privatefiles node when adding it to the navigation, so we need to find it with some overhead
 */
